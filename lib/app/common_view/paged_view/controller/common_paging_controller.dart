...
class CommonPagingController<T> implements Disposable {
  CommonPagingController({
    this.invisibleItemsThreshold = UiConstants.defaultInvisibleItemsThreshold,
    this.firstPageKey = UiConstants.initialPage,
  ...
  void listen({
    ...
    pagingController.addPageRequestListener((pageKey) {
      if (pageKey > UiConstants.initialPage) {
        onLoadMore();
      }
    });
  }
  ...
  //       loadMoreOutput.data,
  //       (pagingController.nextPageKey ?? (UiConstants.initialPage - 1)) + 1,
  //     );
  //   }
  // }
